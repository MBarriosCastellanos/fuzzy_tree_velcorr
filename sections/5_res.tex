...
Table~\ref{tab:trees} reports the out-of-fold metrics of the binomial trees. The 2 [mpy] tree reached a mean recall of 0.710 (0.646 for the non-exceeding class, 0.774 for the exceeding class), and the 8 [mpy] tree reached 0.808 with a recall ratio of 0.986. Figure~\ref{fig:cm} shows the three-class confusion matrix composed from the two out-of-fold binary decisions: the strategy recovers 0.60, 0.47, and 0.75 of the low, medium, and high classes, with the adjacent medium class as the hardest region.
...
\begin{itemize}
  ...
  \item \textbf{R2} (low, $c_2 = 1.2$ [mpy]): mature lines ($t_s > 14$ [yr]) with low gas rate ($Q_g \leq 69$ [MMSCFD]). This is the dominant low rule (46\% of the zone): a line that has survived for more than 14 years under quiet service indicates a benign environment, and the low gas rate limits turbulence and CO$_2$ supply.
  ...
  \item \textbf{R4} (mid, $c_4 = 7.1$): young, slow, small-diameter lines. The ordinal exclusion suppresses this rule completely (0\% contribution): its region belongs to the severe rule R7, demonstrating that the exclusion operates as designed.
...
The out-of-fold parity of the fuzzy system: an $R^2$ of 0.50, an MAE of 3.65 [mpy], and an RMSE of 5.20 [mpy] over the all records. Figure~\ref{fig:response} demonstrates the property promised by the formulation: sweeping the service time with the remaining variables at their medians produces a continuous output that ramps across the normative limits instead of jumping between class labels.
...
Figure~\ref{fig:zones} decomposes the error by severity zone. The fuzzy system improves on linear regression in every zone (3.14 against 4.02 [mpy] in the low zone, 2.82 against 3.30 in the medium zone, and 4.66 against 4.94 in the high zone) and achieves the lowest low-zone error among the four methods. Thresholding the continuous outputs at the normative limits shows why the strategy separates the classification and estimation roles: thresholded linear regression recovers the high class strongly (recall 0.89) only by overpredicting --- its low-class recall collapses to 0.31 --- whereas the dedicated classification layer of the strategy delivers balanced recalls (0.80 and 0.81 at the 8 [mpy] limit).
...
First, its complexity: the trees selected by the inner loops carry between 15 and 45 leaves per fold. Each leaf is one disjoint rule, so the regressor speaks through 15--45 rules, compared to 12 rules with at most 4 conditions. An expert can audit twelve propositions; a forty-leaf partition exceeds the practical limit of rule-by-rule verification.
